...
- **7pm reminders**: browser notification + on-page banner nudging you to do that day's workout
...
2. In **Settings**, set your **Challenge Start Date** (defaults to today) — this is what "Day 1" is calculated from.
3. Click **🔔 Enable 7PM Reminders** and allow notifications when prompted.
...
- While the site is open in a browser tab (even in the background), it checks the time every 30 seconds and will fire a **browser notification** the first time it's past 7pm on a challenge day you haven't completed yet, plus show an in-page reminder banner.
- **The site cannot send you a notification if your browser is fully closed** — true "wake up my phone at 7pm" push notifications would require a backend server + push service. If you want that, keep a browser tab open (or pinned) around 7pm, or consider adding a phone calendar reminder as a backup.
